...
def test_load_yaml_rejects_non_list_max_swaps(tmp_path):
    bad = tmp_path / "bad.yaml"
    bad.write_text("""
random_seed: 0
system: {n_sc: 3, ce: paircut9_5_5_ardr_n96}
...
wl: {flatness_limit: 0.8, fill_factor_limit: 1.0e-12, schedule: "1_over_t",
     ...
     n_trials_per_walker: 1000, block_size_sweeps: 10,
     trajectory_write_interval_sweeps: 0}
config_search: {n_workers: 1, max_swaps: "123", attempts_per_swap_count: 1, random_attempts: 1}
checkpoint: {filename: c.h5, interval_cycles: 0}
""")
    with pytest.raises(ValueError, match="max_swaps must be a list of ints"):
        load_yaml(bad)


def test_load_yaml_rejects_zero_or_negative_max_swaps(tmp_path):
    bad = tmp_path / "bad.yaml"
    bad.write_text("""
random_seed: 0
system: {n_sc: 3, ce: paircut9_5_5_ardr_n96}
...
wl: {flatness_limit: 0.8, fill_factor_limit: 1.0e-12, schedule: "1_over_t",
     ...
     n_trials_per_walker: 1000, block_size_sweeps: 10,
     trajectory_write_interval_sweeps: 0}
config_search: {n_workers: 1, max_swaps: [0, 1], attempts_per_swap_count: 1, random_attempts: 1}
checkpoint: {filename: c.h5, interval_cycles: 0}
""")
    with pytest.raises(ValueError, match="max_swaps entries must all be >= 1"):
        load_yaml(bad)
